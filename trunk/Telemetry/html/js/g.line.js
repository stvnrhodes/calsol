...
Raphael.fn.g.linechart = function (x, y, width, height, valuesx, valuesy, opts) {
    ...
    var allx = Array.prototype.concat.apply([], valuesx),
        ...
        maxx = xdim.to;
    if (opts.y_limits) {
        var _miny = opts.y_limits[0];
        var _maxy = opts.y_limits[1];
    } else {
        var _miny = Math.min.apply(Math, ally);
        var _maxy = Math.max.apply(Math, ally);
    }
    var ydim = this.g.snapEnds(_miny, _maxy, valuesy[0].length - 1),
        miny = ydim.from,
        maxy = ydim.to,
