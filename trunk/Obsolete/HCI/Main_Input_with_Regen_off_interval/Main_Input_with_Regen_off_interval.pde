...
#define OFF_REGEN_MIN 6.5  //do not mix up min or max, or you will 
#define OFF_REGEN_MAX 10.5 //NEVER brake with regen
...
void accel(){
  ...
  if(brake > BRAKE_THRESHOLD) {
    float currentmph = recordedSpeed * 2.2369;
    setspeed = 0.0;
    brakeOn = true;
    if ((currentmph > OFF_REGEN_MIN)&&(currentmph<OFF_REGEN_MAX)){
    voltage = 0;
    }
    else{
